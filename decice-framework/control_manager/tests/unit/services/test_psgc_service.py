        ...
        "placements": [
            {
                "task_id": str(
                    uuid.uuid4()
                ),  # --- [FIX] Updated key from workload_id ---
...
class TestSchedulingService:
    ...
    async def test_record_scheduling_results_handles_db_error(
        ...
        mock_logger.exception.assert_called_once_with(
            f"Failed to record scheduling decisions: DB Connection Failed",
            exc_info=True,
        )
...
#         assert created_decisions[0].__class__.__name__ == "SchedulingDecision"

#         assert created_decisions[0].task_id == uuid.UUID(
#             scheduler_response_json["placements"][0]["workload_id"]
#         )
