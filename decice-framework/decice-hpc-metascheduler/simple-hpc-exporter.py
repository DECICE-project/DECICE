...
class SimpleHPCCollector:
    ...
    def parse_sinfo(self):
            ...
            if line.strip():
                ...
                if len(parts) >= 6:
                    ...
                    try:
                        ...
                        if len(cpu_parts) == 4:
                            cpu_allocated, cpu_idle, cpu_other, cpu_total = map(
                                int, cpu_parts
                            )
                    ...
                    partitions[partition][state] = (
                        partitions[partition].get(state, 0) + nodes
                    )
        ...
        node_utilization = (
            (total_stats.get("alloc", 0) + total_stats.get("mix", 0))
            / max(total_stats["total"], 1)
        ) * 100
        cpu_utilization = (total_cpus["allocated"] / max(total_cpus["total"], 1)) * 100

        # Calculate memory utilization
        total_memory["allocated_mb"] = (
            total_memory["total_mb"] - total_memory["free_mb"]
        )
        memory_utilization = (
            total_memory["allocated_mb"] / max(total_memory["total_mb"], 1)
        ) * 100
    ...
    def generate_prometheus_metrics(self):
        ...
        lines.append(
            "# HELP remote_hpc_cpu_utilization_percent CPU utilization percentage"
        )
        lines.append("# TYPE remote_hpc_cpu_utilization_percent gauge")
        lines.append(
            f'remote_hpc_cpu_utilization_percent{{host="{host}"}} {cluster.get("cpu_utilization", 0)}'
        )

        lines.append(
            "# HELP remote_hpc_memory_utilization_percent Memory utilization percentage"
        )
        lines.append("# TYPE remote_hpc_memory_utilization_percent gauge")
        lines.append(
            f'remote_hpc_memory_utilization_percent{{host="{host}"}} {cluster.get("memory_utilization", 0)}'
        )

        # Legacy/compatibility metrics
        lines.append(
            f'remote_hpc_utilization_percent{{host="{host}"}} {cluster.get("utilization", 0)}'
        )
        lines.append(
            f'remote_hpc_nodes_total{{host="{host}"}} {cluster.get("total_nodes", {}).get("total", 0)}'
        )
        lines.append(
            f'remote_hpc_nodes_idle{{host="{host}"}} {cluster.get("available", 0)}'
        )
        ...
        lines.append(
            f'remote_hpc_cpu_allocated{{host="{host}"}} {cpu_stats.get("allocated", 0)}'
        )
        lines.append(f'remote_hpc_cpu_idle{{host="{host}"}} {cpu_stats.get("idle", 0)}')
        lines.append(
            f'remote_hpc_cpu_total{{host="{host}"}} {cpu_stats.get("total", 0)}'
        )

        # Detailed Memory breakdown
        memory_stats = cluster.get("memory_stats", {})
        lines.append(
            f'remote_hpc_memory_allocated_mb{{host="{host}"}} {memory_stats.get("allocated_mb", 0)}'
        )
        lines.append(
            f'remote_hpc_memory_free_mb{{host="{host}"}} {memory_stats.get("free_mb", 0)}'
        )
        lines.append(
            f'remote_hpc_memory_total_mb{{host="{host}"}} {memory_stats.get("total_mb", 0)}'
        )

        # Job metrics
        lines.append(f'remote_hpc_jobs_total{{host="{host}"}} {jobs.get("total", 0)}')
        lines.append(
            f'remote_hpc_jobs_running{{host="{host}"}} {jobs.get("running", 0)}'
        )
        lines.append(
            f'remote_hpc_jobs_pending{{host="{host}"}} {jobs.get("pending", 0)}'
        )
