...
class PrometheusMetricsClient:
    ...
    def __init__(self, prometheus_url: str = "http://prometheus-kube-prometheus-prometheus.monitoring.svc.cluster.local:9090"):
    ...
    async def get_volcano_metrics(self) -> Dict:
        ...
        if memory_data and memory_data.get("result"):
            memory_bytes = sum(float(result["value"][1]) for result in memory_data["result"])
        ...
        if cluster_memory_data and cluster_memory_data.get("result"):
            metrics["memory_percent"] = round(float(cluster_memory_data["result"][0]["value"][1]), 2)
        else:
    ...
    async def get_hpc_metrics(self) -> Dict:
        ...
        cpu_data = await self.query_prometheus(cpu_query)
        if cpu_data and cpu_data.get("result"):
            metrics["cpu_utilization_percent"] = float(cpu_data["result"][0]["value"][1])
        
        memory_query = 'remote_hpc_memory_utilization_percent{host="192.168.23.14"}'
        memory_data = await self.query_prometheus(memory_query)
        if memory_data and memory_data.get("result"):
            metrics["memory_utilization_percent"] = float(memory_data["result"][0]["value"][1])
    ...
    async def get_all_scheduler_metrics(self) -> Dict:
        ...
        volcano_metrics, hpc_metrics = await asyncio.gather(
            self.get_volcano_metrics(),
            self.get_hpc_metrics()
        )
...
class IntelligentSchedulerService:
    ...
    async def get_scheduling_recommendation(self, job_data: Dict) -> str:
        ...
        try:
            ...
            decision_factors = self._analyze_scheduling_factors(volcano_metrics, hpc_metrics)
    ...
    def _analyze_scheduling_factors(self, volcano_metrics: Dict, hpc_metrics: Dict) -> Dict:
        ...
        if prefer_hpc:
            factors["reasons"].append(f"HPC is less loaded (score: {hpc_load_score:.1f} vs {volcano_load_score:.1f})")
        else:
            factors["reasons"].append(f"Volcano is preferred (score: {volcano_load_score:.1f} vs {hpc_load_score:.1f})")
    ...
    def _calculate_hpc_load_score(self, hpc_metrics: Dict) -> float:
        ...
        queue_score = min(jobs_pending * 10, 100.0) * 0.2  # Each pending job adds 10 points, max 100
    ...
    def _get_load_details(self, volcano_metrics: Dict, hpc_metrics: Dict) -> List[str]:
        ...
        details.append(f"Volcano: CPU {volcano_cpu:.1f}%, Memory {volcano_memory:.1f}%, Queue {volcano_queue}")
        ...
        details.append(f"HPC: Utilization {hpc_util:.1f}%, Pending {hpc_pending}, Available nodes {hpc_nodes_avail}/{hpc_nodes_total}")
