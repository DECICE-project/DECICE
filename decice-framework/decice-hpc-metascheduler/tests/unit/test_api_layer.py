...
def test_submit_job_api_success(client: TestClient):
    ...
    """
    # Arrange: We use `patch` to intercept calls to the UnitOfWork and service layer.
    ...
    with patch("app.SchedulerService") as mock_scheduler_service:
        ...
        mock_scheduler_service.return_value.submit_job.return_value = (
            mock_job_domain_model
        )
        ...
        with patch("app.UnitOfWork") as mock_uow:
            ...
            # Assert: Verify the API layer's behavior
