...
class JobRepository:
    ...
    def add(self, name: str, image: str, scheduler_target: str, user_id: str) -> Dict[str, Any]:
        """
        ...
        new_job_model = JobModel(
            name=name,
            image=image,
            target_cluster=scheduler_target,
            user_id=user_id
        )
    ...
    def get(self, job_id: UUID, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        job_model = self.session.query(JobModel).filter_by(
            id=job_id,
            user_id=user_id
        ).first()
    ...
    def list(
        ...
        if name:
            query = query.filter(JobModel.name.ilike(f"%{name}%")) # Case-insensitive substring match
            
        job_models = query.order_by(JobModel.creation_timestamp.desc()).offset(offset).limit(limit).all()
        
        return [job.to_dict() for job in job_models]

    def update(self, job_id: UUID, user_id: str, update_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    def count(self, user_id: str, status: Optional[str] = None, name: Optional[str] = None) -> int:
        """
