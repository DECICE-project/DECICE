    ...
    while True:
        try:
            ...
            anomaly_data = {
                ...
                "LSTM": bool(
                    is_anomaly_rate_below_threshold(
                        evaluate_lstm_detect_anomalies(temp, ["CPU Usage", "Memory Usage", "IO Usage"], log_dir)
                    )
                ),
                "CNN": bool(
                    is_anomaly_rate_below_threshold(CNN_Anomaly_detect(temp, ["CPU Usage", "Memory Usage", "IO Usage"]))
                ),
                "Z-Score": bool(
                    is_anomaly_rate_below_threshold(calculate_zscores(temp, ["CPU Usage", "Memory Usage", "IO Usage"]))
                ),
                "ARIMA": bool(
                    is_anomaly_rate_below_threshold(
                        detect_anomalies_arima(temp, ["CPU Usage", "Memory Usage", "IO Usage"])
                    )
                ),
                "Isolation Forest": bool(
                    is_anomaly_rate_below_threshold(
                        detect_anomalies_isolation_forest(temp, ["CPU Usage", "Memory Usage", "IO Usage"])
                    )
                ),
                "One-Class SVM": bool(
                    is_anomaly_rate_below_threshold(
                        detect_anomalies_one_class_svm(temp, ["CPU Usage", "Memory Usage", "IO Usage"])
                    )
                ),
            }
