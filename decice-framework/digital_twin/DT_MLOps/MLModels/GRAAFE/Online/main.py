...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--rack_name",
        type=str,
        help="Rack Name, all mesans all racks of the M100 one by one in serial approach. ",
        default="r256",
    )
    ...
    parser.add_argument(
        ...
        "--broker_address",
        type=str,
        help='broker_address = "192.168.0.35" or broker_address = "examon.cineca.it"',
    ...
    if rack_name == "all":
        ...
        while True:
            ...
            try:
                ...
                start_time = logging_module.start_time(
                    f"Room Start ==> first preprocessing {df_raw.shape}, {df_just_extracted.shape}", logging
                )
                ...
                for i, rack_name in enumerate(
                    set([i.split("n")[0] for i in pd.read_csv("node_names").node_name.values])
                ):
                    try:
                        print("----------- II Preprocessing -----------")
                        start_time = logging_module.start_time(
                            f"{i} - {rack_name} ==> (II) preprocessing_rack", logging
                        )
