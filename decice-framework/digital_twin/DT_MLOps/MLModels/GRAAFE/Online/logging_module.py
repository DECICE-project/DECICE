...
log_file_name = (
    "log"
    + "-"
    + datetime.datetime.strftime(datetime.datetime.now(pytz.timezone("Europe/Rome")), "%Y-%m-%d")
    .replace(":", "-")
    .replace(" ", "-")
    + ".log"
)
...
logging.basicConfig(filename=os.path.join(loggin_path, log_file_name), level=logging.INFO, format=formatter)
