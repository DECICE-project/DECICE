...
class ExamonQL(object):
    ...
    def __init__(self, examon):
        ...
        self.jc = JobsClient(
            self.ex.host, user=self.ex.user, password=self.ex.password, verbose=self.ex.verbose, comp=self.ex.comp
        )
        self.ds = ExDask(self.ex)
        self.get_tag_keys()
    ...
    def get_tag_keys(self, max_workers=THREADPOOL_WORKERS, cache=True):
        ...
        if not self.tag_list:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_metrics = {
                    executor.submit(self.ex.query_metricstags, m, None, None, cache): m
    ...
    def DESCRIBE(self, metric=None, tag_key=None, tag_value=None, where=None):
        ...
        where : str
            ...
            If separator character ``','`` is present in ``tag_value``, it must be
            escaped: ``'\,'``.
        ...
        if (metric == None) and (tag_key == None) and (tag_value == None) and (where == None):
            ...
            if "," in value:
                # value = value.split(',')
                value = [each.replace("\,", ",") for each in re.split(r"(?<!\\),", value)]
    ...
    def WHERE(self, **kwargs):
        ...
        for k, v in kwargs.items():
            ...
            if "," in v:
                # value = v.split(',')
                value = [each.replace("\,", ",") for each in re.split(r"(?<!\\),", v)]
                value = [x.strip() for x in value]
            else:
                value = [v.strip()]
    ...
    def AGGRBY(
        self, name, align_sampling=True, align_start_time=False, sampling_value=1, sampling_unit="minutes", **kwargs
    ):
        ...
        if name:
            aggr = {
                ...
                "sampling": {"value": sampling_value, "unit": sampling_unit},
            }
        else:
            raise ValueError("Wrong arguments number")
    ...
    def __handle_wildcards(self):
        ...
        if query.groupby:
            if "*" in query.groupby:
                ...
                groups = [{"name": "tag", "tags": tags} if x == "*" else x for x in groups]
                query = query._replace(groupby=groups)
        return query
    ...
    def execute_async(self, batch_size=320000, npartitions=200, shuffle=False, **kwargs):
        query = self.get_query()
        ret = None
        if set(query.metrics).issubset(self.jc.JOB_TABLES):  # check if a job table. TODO: better integration
            ret = self.jc.query_jobs_async(query)
        else:
            # raise NotImplementedError
            columns = []
            for g in query.groupby:
                if g["name"] == "tag":
                    columns.extend([x for x in g["tags"]])
            # columns = [x for x in query.groupby['tags']]
            columns.append("name")
            meta = {a: "object" for a in columns}
            meta["timestamp"] = "int64"
            meta["value"] = "object"

            self.ds.q_slices = []
            df = self.ds.dask(**kwargs).query(*query, batch_size=batch_size)
            if shuffle:
                df = df.shuffle()
            df = df.to_bag(npartitions=npartitions).flatten().to_dataframe(meta=meta).persist()
            ret = df.categorize(columns=columns).compute()
        return ret
...
class Examon(KairosDb):
    ...
    def to_series(self, columns=None, flat_index=False, interp=None, dropna=False):
        ...
        self.df_ts = pd.pivot_table(
            self.df_table, index=["timestamp"], columns=columns, values="value", aggfunc="first"
        )

        if len(columns) > 1:
            if flat_index is True:
                self.df_ts.columns = self.__to_labels(
                    list(self.df_ts.columns.names), self.df_ts.columns.values.tolist()
                )
    ...
    def to_table(self):
        ...
        self.df_table = pd.DataFrame(self.to_dict())
        if len(self.df_table):
            self.df_table["timestamp"] = (
                pd.to_datetime(self.df_table["timestamp"], unit="ms")
                .dt.tz_localize(tz="UTC")
                .dt.tz_convert(tz=self.time_zone)
            )
            # --------- use categoricals
    ...
    #                     if g['name']=='tag':
    #                         row = {k:v for k,v in g['group'].iteritems()}
    #                         break
    #                 if len(row) == 0:
    #                     row = {k:v[0] for k,v in results['tags'].iteritems()}
    #                 row_tv = [{'timestamp': t[0], 'value': t[1], 'name': results['name']} for t in results['values']]
    #                 for t in row_tv:
    #                     t.update(row)
    #                 self.dict_table.extend(row_tv)
    ...
    def get_utctmp(self, timestring, time_zone, format="%d-%m-%Y %H:%M:%S"):
        ...
        dt_epoch = (
            tz.normalize(tz.localize(dt)).astimezone(pytz.utc) - datetime.datetime(1970, 1, 1).replace(tzinfo=pytz.UTC)
        ).total_seconds()
    ...
    def query_tojson(
        self, tstart, tstop, metric_names, tags=None, groupby=None, aggrby=None, limit=None, time_zone="Europe/Rome"
    ):
        """Query builder"""
    ...
    def exec_query(self, query_json):
        ...
        return copy.deepcopy(
            self
        )  # FIXME: <<<<<<<<<<<<<<<<<<<< TODO return self: build a new object at every query (modfify this in doc)
