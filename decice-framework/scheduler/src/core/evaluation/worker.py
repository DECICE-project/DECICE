...
def run_evaluation_task(job_id: str, scheduler_name: str, dataset_name: str):
    ...
    logger.info(f"Eval Worker {job_id} started. Model: {scheduler_name} vs Data: {dataset_name}")
    ...
    try:
        ...
        if not model_path.exists(): raise FileNotFoundError(f"Model {scheduler_name} not found")
        if not dataset_path.exists(): raise FileNotFoundError(f"Dataset {dataset_name} not found")
        ...
        for sc_file in scenarios:
            try:
                with open(sc_file, "r") as f: data = json.load(f)
                ...
                # B. AI Prediction
                ai_choice = ai_scheduler.predict_best_strategy_name(req, deterministic=True)
                if not ai_choice: ai_choice = "round_robin" # Fallback
        ...
        if df.empty: raise RuntimeError("No valid results generated.")
